...
def GMM_fitting(int_raster):
    ...
    for i in range(0, x.shape[0]):
        if p[2, i] <= p[1, i] and flag_wet == 0:
            flag_wet = 1
            print('wet transition = ', x[i])
            wet_tras = x[i]
        if p[1, i] <= p[0, i] and flag_dry == 0:
            flag_dry = 1
            print('dry transition = ', x[i])
            dry_tras = x[i]
...
def veg_fix(veg_raster, out_veg_raster, area_thresh, pix_per_m):
    ...
    bin_veg, _ = delete_small_area(veg, area_thresh, pix_per_m)
...
def make_wet_bound(int_raster, edge_raster, out_wet_raster, area_thresh, int_thresh, pix_per_m):
    ...
    bin_wet, area_wet = delete_small_area(wet, area_thresh, pix_per_m)
...
def disconnect_valley_segments(valley, flowdir, size, order):
    ...
    order = arcpy.RasterToNumPyArray(order, nodata_to_value=0).astype(np.uint8)
...
def find_sep_border(a, size, corner, dx):
    ...
    del Lab
    common_border = arcpy.RasterToNumPyArray(
        FocalStatistics(comm_raster, NbrRectangle(size, size, "CELL"), "VARIETY", "DATA"), nodata_to_value=0)
    common_border = np.where(common_border >= 2, 1, 0).astype(np.uint8)
...
def delete_small_area(A, area_thresh, pix_per_m):
